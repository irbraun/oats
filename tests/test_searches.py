...
def test_binary_search_rabin_karp(text, pattern, q, expected):
	from oats.nlp.search import binary_robinkarp_match
	assert binary_robinkarp_match(pattern, text, q) == expected
...
def test_search_for_any_rabin_karp(text, patterns, q, expected):
	from oats.nlp.search import any_rabinkarp_matches
	assert any_rabinkarp_matches(patterns, text, q) == expected
...
def test_search_for_all_rabin_karp(patterns, text, q, expected):
	from oats.nlp.search import all_rabinkarp_matches
	assert all_rabinkarp_matches(patterns, text, q) == expected
...
def test_binary_search_fuzzy(text, pattern, threshold, local, expected):
	from oats.nlp.search import binary_fuzzy_match
	assert binary_fuzzy_match(pattern, text, threshold, local) == expected
...
def test_search_for_any_fuzzy(text, patterns, threshold, local, expected):
	from oats.nlp.search import any_fuzzy_matches
	assert any_fuzzy_matches(patterns, text, threshold, local) == expected
...
@pytest.mark.parametrize("text, patterns, threshold, local, expected", [
	("Mature seeds from TCDD-treated plants had a characteristic 'wrinkled' phenotype.", ["seeds"], 0.99, 1, ["seeds"]),                    # A perfect local match is found when only one pattern is passed in.
	("Mature seeds from TCDD-treated plants had a characteristic 'wrinkled' phenotype.", ["seeds","s_eds"], 0.99, 1, ["seeds"]),            # When the threshold is 1, only the perfect local match is returned.
...
def test_search_for_all_fuzzy(text, patterns, threshold, local, expected):
	from oats.nlp.search import all_fuzzy_matches
	assert all_fuzzy_matches(patterns, text, threshold, local) == expected
